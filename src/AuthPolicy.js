'use strict'
// Taken liberally from the AWS API Gateway nodejs authorizer blueprint
...
class AuthPolicy {
   ...
   */
  static _getEmptyStatement (effect) {
    return {
   ...
   */
  static _getStatementsForEffect (effect, methods) {
    const statements = []

    if (methods.length > 0) {
      const statement = AuthPolicy._getEmptyStatement(effect)
      ...
      for (let i = 0; i < methods.length; i++) {
        ...
        if (curMethod.conditions === null || curMethod.conditions.length === 0) {
          ...
          const conditionalStatement = AuthPolicy._getEmptyStatement(effect)
  ...
  build () {
    ...
    return {
      ...
      policyDocument: {
        ...
        Statement: []
          .concat(AuthPolicy._getStatementsForEffect('Allow', this.allowMethods))
          .concat(AuthPolicy._getStatementsForEffect('Deny', this.denyMethods))
      }
    }
  }
}
